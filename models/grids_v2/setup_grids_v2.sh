#!/bin/bash
#
# setup_grids_v2.sh - v2 grid (2026-09-24), built from grids/setup_grids.sh. Differences from v1:
#   - template_v2 (IGW damping columns, FeCZ finder on mlt_mixing_type, v from mlt_vc not conv_vel,
#     triggered profiles); Asplund+09 mixture + opacities (inlist_base);
#   - rotating models: surf_omega_div_omega_crit_limit = 0.98, mdot redo, no lgTeff/lgL hard limits
#     (test T2v2 in /home/mcantiello/rednoise_tests);
#   - separate Ceph tree (grids_v2) and job names rn2_*; sub-grids without inlist_common are skipped.
#
...
#
# Usage:
#   ./setup_grids_v2.sh MW        # Set up all 4 rotation rates for MW (the v2 pilot)
#   ./setup_grids_v2.sh MW w0.2   # Set up only MW/w0.2
#
# Prerequisites:
#   - template_v2/star must be compiled (cd ../template_v2 && ./mk)
...
TEMPLATE="$MODELS_DIR/template_v2"
CEPH_BASE="/mnt/ceph/users/mcantiello/rednoise/grids_v2"
...
write_inlist_grid_rot() {
    # Mirrors the v1 files actually run in grids/*/w0.[246] (hand-edited after v1 setup:
    # rotation switched on near the ZAMS; new_omega_div_omega_crit is a star_job option),
    # with the v1 "rotational mass loss feedback" block replaced by the v2 controls.
    local model_dir="$1"
    local mass="$2"
    local omega="$3"
    cat > "$model_dir/inlist_grid" << GRID_EOF
...
&star_job
      ...
      ! Rotation setup: keep rotation off until near ZAMS,
      ! then near_zams_relax enables it and relaxes to target omega
...
&controls
      ...
      ! Rotational mass loss feedback. v2: keep near-critical rotators evolving past the
      ! TAMS (v1 w0.6 runs died on min_timestep_limit; v1 had redo_cnt 5, limit 0.99,
      ! revise 1.1, boost 0.1). Also drop the Teff/L hard limits from inlist_base.
      surf_omega_div_omega_crit_limit = 0.98d0
      surf_omega_div_omega_crit_tol = 0.02d0
      max_mdot_redo_cnt = 200
      mdot_revise_factor = 1.2d0
      implicit_mdot_boost = 0.05d0
...
for zi in "${!Z_LABELS[@]}"; do
    ...
    if [ ! -f "$z_dir/inlist_common" ]; then
        [ "$FILTER_Z" = "$z_label" ] && { echo "ERROR: $z_dir/inlist_common not found."; exit 1; }
        echo "SKIP: $z_label (no inlist_common)"
        continue
    fi
