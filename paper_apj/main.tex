...
\emph{Amplitude budget.} Three-dimensional simulations of waves excited by the convective core predict surface photometric amplitudes of 0.06~$\umag$ for a non-rotating 15~$M_\odot$ star, rising to 0.21~$\umag$ with rotation \citep{anders2023}. The observed 10--90\% range on the primary sample is 64--3600~$\umag$ with median 350~$\umag$; the shortfall is 3--4 orders of magnitude and no plausible correction to the simulations closes it. The linear transfer function that converts the convective wave flux into a surface signal has itself been verified against direct simulations \citep{lecoanet2021}, so the deficit is not an artifact of the propagation calculation.

\emph{Direct simulations of the FeCZ.} Three-dimensional radiation-hydrodynamic simulations of envelope segments, which resolve the FeCZ and the layers above it, reproduce the photospheric velocities, the spectral slope, and the amplitudes without free parameters; their characteristic frequencies are discussed in Section~\ref{sec:mesa}. Convection from the iron peak overshoots to the photosphere, so that photospheric velocities reflect those in the zone: $\approx$150~$\kms$ in a mid-main-sequence $35\,M_\odot$ model, 19~$\kms$ in a $13\,M_\odot$ model at the TAMS, and a 10--70~$\kms$ distribution (median 30--40~$\kms$) in a $20\,M_\odot$ blue supergiant whose observed counterpart has $\Theta\approx40~\kms$ \citep{schultz2022,schultz2023,ma2026}. The light curves show SLF variability with $\gamma=1.9\pm0.2$ \citep{schultz2022}, the near-universal slope we measure, and amplitudes that, once reduced by $\sqrt{n}$ for the $n$ uncorrelated patches needed to tile the visible disk, fall in the observed range (0.03--0.2\% in the $35\,M_\odot$ model, $\approx130~\umag$ in the $13\,M_\odot$ one; \citealt{schultz2022,schultz2023}).
...
(ii) \emph{Saturation.} The models do not reproduce it. The FeCZ velocity keeps rising steeply through $\lgL=3.7$ ($\partial\log v_{c,\max}/\partial\lgL=+1.56$ below and $+1.72$ above, against $+0.81$ and $+0.29$ for $\vmac$), and it is the flux fraction that flattens ($+3.58$ to $+2.39$), the opposite of the observed pattern. The model velocity reaches the observed $\vmac$ only at $\lgL\approx4.0$--4.2; below that, $\vmac$ exceeds $v_{c,\max}$ by factors of 3 to $\approx15$ (Figure~\ref{fig:mesa}a). A saturation of the photospheric velocity field at $\lgL\approx3.7$ must therefore arise between the FeCZ and the photosphere rather than in the MLT velocity itself. The 3D simulations locate that coupling: there is no quiescent radiative layer to transfer through, because the FeCZ convection overshoots to the photosphere, and the photospheric velocities track the rms velocities in the zone \citep{schultz2023,ma2026}. They also identify two effects absent from MLT that grow toward the Eddington limit: radiative losses from rising plumes once the zone lies above the critical optical depth $\tau_{\rm crit}\sim cP_{\rm rad}/[(P_{\rm rad}+P_{\rm gas})v_c]$, which make convection inefficient \citep{jiang2015}, and turbulent pressure, which supplies up to 80\% of the support when the motions become transsonic \citep{schultz2023}. Velocities approaching the sound speed would naturally cap the photospheric velocity field. The flux fraction reaches $10^{-2}$ at $\lgL=3.68$, at the observed saturation point.
...
\emph{Frequencies.} The FeCZ turnover frequency lies below the observed $\nuchar$ by a factor of 3.1 (median 0.49 dex, 16--84\%: 0.33--0.62 dex, over the trustworthy cells with $\lgT>4.3$), recovering the offset of \citet{cantiello2021}. The 3D simulations bracket the observations from the other side: their $\nuchar$ matches the observed value for a $13\,M_\odot$ star at the TAMS (1.5~$\dinv$), but in the more luminous $35\,M_\odot$ and blue-supergiant models it is set by the thermal time of the FeCZ and exceeds the observed values by a factor of 2--3 \citep{schultz2022,schultz2023,ma2026}. The observed frequency therefore lies between the MLT turnover frequency and the thermal frequency of the zone. The offset does not depend on $\ell$ ($+0.06$~dex per dex), and on the main sequence the model reproduces the observed $\teff$ dependence ($\partial\log\nu_c/\partial\lgT=+1.75$ [1.46, 2.00] against $+1.53$) and the near-absence of an $\ell$ dependence ($+0.02$ against $-0.23$). Across the full map, however, the offset grows toward cooler $\teff$ ($-1.1$~dex per dex in $\lgT$; Figure~\ref{fig:mesa}c), because the model $\nu_c$ falls steeply once the FeCZ deepens after the main sequence. At fixed mass the models reproduce the signs of all three evolutionary trends but overshoot the amplitudes: $\partial/\partial\tau=+0.85$ for $\log v_{c,\max}$ and $+2.64$ for $\log F_c/F$ (observed $+0.27$ for $\vmac$ and $+1.73$ for $\alpha_0$), and $-0.12$ [$-0.17$, $-0.06$] for $\log\nu_c$ ($-0.33$ observed).

\emph{Cool side.} The handover interpretation of Section~\ref{sec:cool_disc} is not supported with this definition of the turnover time. In the evolved OB/BSG region ($4.0<\lgT<4.3$, $\lgL>3.6$) the models' FeCZ turns over at 0.018 [0.014, 0.027]~$\dinv$ and the He\,II zone at $7\times10^{-4}~\dinv$, one to three orders of magnitude below the observed 0.27~$\dinv$; in the yellow-supergiant region the FeCZ, He\,II, and H zones give 0.007, 0.010, and 0.014~$\dinv$ against 0.30~$\dinv$ observed. The failure is one of the 1D turnover time rather than of the sub-surface picture: a 3D simulation of a $20\,M_\odot$ blue supergiant finds the Fe and He zones merged into a single convective region that reaches the photosphere, with SLF variability peaking at 10--20~$\mu$Hz (0.9--1.7~$\dinv$), within a factor of $\approx3$ of its observed counterpart \citep{ma2026}. The same simulation leaves no radiative cavity between the zones in which gravity waves could be trapped.
...
In summary, a single grid with $\alpha_{\rm MLT}=1.6$ reproduces the onset as a threshold effect, the $\teff$ scaling and $\ell$-independence of the turnover frequency up to a constant factor of $\approx3$ on the main sequence, and the sign and ordering of every evolutionary trend and sensitivity ratio. It does not reproduce the saturation of $\vmac$, the magnitude of the sensitivity ratios (low by a factor 1.6--2.5), the cool-side frequencies, or the SMC stars below the model onset. The sub-surface interpretation is therefore supported in its structure but not yet quantitatively; the failures point at the coupling between the FeCZ and the photosphere, which one-dimensional models do not describe. Averaged 3D envelopes agree with MESA structures in the weak-turbulence regime, where the zone is radiatively lossy or the luminosity is well below Eddington, but not closer to the Eddington limit, where density--flux correlations reduce the radiation-pressure gradient and turbulent pressure modifies the temperature gradient \citep{schultz2020,schultz2023}. The MESA comparison is therefore most reliable for the lower half of our luminosity range, and the saturation regime is exactly where it is expected to fail.
...
\input{tables/tab_samples}
\input{tables/tab_vmac}
\input{tables/tab_fits}
\input{tables/tab_onset}
\input{tables/tab_regions}
\input{tables/tab_mesa}
